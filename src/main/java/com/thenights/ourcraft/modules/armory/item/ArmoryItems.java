/*
 * To change this license header, choose License Headers in Project Properties.
 * To change this template file, choose Tools | Templates
 * and open the template in the editor.
 */
package com.thenights.ourcraft.armory.item;

import net.minecraft.item.Item;

/**
 *
 * @author Stephanie
 */
public class ArmoryItems {
     // =========================================
     // WOOD
     // Weapons
     public static Item WOOD_KATANA;
     // Armor
     public static Item WOOD_HELMET;
     public static Item WOOD_CHEST;
     public static Item WOOD_LEGS;
     public static Item WOOD_BOOTS;
     // =========================================
     // BONE
     // Tools
     public static Item BONE_PICKAXE;
     public static Item BONE_AXE;
     public static Item BONE_SHOVEL;
     public static Item BONE_HOE;
     // Weapons
     public static Item BONE_SWORD;
     public static Item BONE_KATANA;
     // Armor
     public static Item BONE_HELMET;
     public static Item BONE_CHEST;
     public static Item BONE_LEGS;
     public static Item BONE_BOOTS;
     // =========================================
     // Bamboo
     // Tools
     public static Item bamboopickaxe;
     public static Item bambooaxe;
     public static Item bamboospade;
     public static Item bamboohoe;
     // Weapons
     public static Item bamboosword;
     public static Item bambookatana;
     // Armor
     public static Item bamboohelmet;
     public static Item bamboochestplate;
     public static Item bambooleggings;
     public static Item bambooboots;
     // =========================================
     // CLOTH
     // Armor.
     public static Item clothwhitehelmet;
     public static Item clothwhitechestplate;
     public static Item clothwhiteleggings;
     public static Item clothwhiteboots;
     // =========================================
     // Lead
     public static Item leadhelmet;
     public static Item leadchestplate;
     public static Item leadleggings;
     public static Item leadboots;
     // =========================================
     // Emerald
     // Tools
     public static Item emeraldpickaxe;
     public static Item emeraldaxe;
     public static Item emeraldspade;
     public static Item emeraldhoe;
     // Weapons
     public static Item emeraldsword;
     public static Item emeraldkatana;
     // Armor
     public static Item emeraldhelmet;
     public static Item emeraldchestplate;
     public static Item emeraldleggings;
     public static Item emeraldboots;
     // =========================================
     // Obsidian
     // Tools
     public static Item obsidianpickaxe;
     public static Item obsidianaxe;
     public static Item obsidianspade;
     public static Item obsidianhoe;
     // Weapons
     public static Item obsidiansword;
     public static Item obsidiankatana;
     // Armor
     public static Item obsidianhelmet;
     public static Item obsidianchestplate;
     public static Item obsidianleggings;
     public static Item obsidianboots;
     // =========================================
     // Turtle
     // Armor
     public static Item turtlechestplate;
     public static Item turtleleggings;
     public static Item turtleboots;
     // =========================================
     // Firearms
     public static Item flintlockAmmo;
     public static Item blunderAmmo;
     public static Item gunstock;
     public static Item musketBarrelAssembly;
     public static Item blunderBarrelAssembly;
     public static Item musket;
     public static Item pistol;
     public static Item blunderbuss;
     public static Item rifle;
     public static Item revolver;
     public static Item leverActionRifle;
     public static Item leverActionShootgun;
     // public static Item bullet;
     // =========================================
     // Powerup
     public static Item bunnyBoots;
     public static Item floppyBoi;
     // =========================================
     // Vanilie
     public static Item diamondkatana;
     public static Item goldkatana;
     public static Item Ironkatana;
     public static Item stonekatana;

}
